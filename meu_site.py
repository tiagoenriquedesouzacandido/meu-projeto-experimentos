...
# Dia, Hora:Min:Seg, Milissegundos com frações
# ============================================================
def parse_tempo(time_raw):
    ...
    try:
        ...
        # partes[0] = dia (ignorado para Δt dentro do mesmo dia)
        # partes[1] = HH:MM:SS
        # partes[2] = milissegundos fracionários
        hms = partes[1].split(":")
        h, m, s = int(hms[0]), int(hms[1]), int(hms[2])
        ms = float(partes[2]) / 1000.0  # converte ms para segundos
        total = h * 3600 + m * 60 + s + ms
        return total
    except:
        return None
...
def calcular_features(dados, fs, threshold_mv=0.5):
    ...
    # Aqui usamos a banda de 1 kHz a 5 kHz como exemplo (ajuste conforme o ensaio)
    banda_inf = 1000   # Hz
    banda_sup = 5000   # Hz
    ...
    if len(espectro_inicio) > 0 and np.sum(espectro_inicio) > 0:
        freq_iniciacao = freqs_inicio[np.argmax(espectro_inicio)]
    else:
        freq_iniciacao = 0
    ...
    if len(espectro_rev) > 0 and np.sum(espectro_rev) > 0:
        freq_reverbacao = freqs_inicio[np.argmax(espectro_rev)]
    else:
        freq_reverbacao = 0
    ...
    if pot_total > 0:
        freq_pico_pond = np.sum(freqs_pos * espectro_pos**2) / pot_total
    else:
        freq_pico_pond = 0
...
def calcular_amortecimento(dados, fs):
    """
    Decremento logarítmico: mede quão rápido o sinal decai.
    ζ (zeta) próximo de 0 → pouco amortecimento; próximo de 1 → muito amortecido.
    """
...
if not os.path.exists(PASTA_RAIZ):
    st.error(f"Pasta '{PASTA_RAIZ}' não encontrada. Crie a pasta e adicione os experimentos.")
else:
    ...
    if not subpastas:
        st.warning("Nenhum experimento encontrado na pasta 'experimentos'.")
        st.stop()

    # Sidebar: seleção do modo de visualização
    modo = st.sidebar.radio("Selecione o Modo:", ["🔬 Visão Detalhada", "🆚 Comparação", "📚 Biblioteca de Sinais"])

    st.sidebar.markdown("---")

    # Sidebar: configuração do threshold (em mV, pois os dados são em mV)
    st.sidebar.subheader("⚙️ Parâmetros de Análise")
    threshold_mv = st.sidebar.number_input("Threshold (mV)", min_value=0.01, max_value=100.0, value=0.5, step=0.1)

    # Sidebar: velocidade de propagação por material (para cálculo de distância via ToA)
    st.sidebar.markdown("---")
    ...
    # Analisa um experimento por vez, mostrando gráficos e tabela de features
    # ============================================================
    if modo == "🔬 Visão Detalhada":
        ...
        for arq in arquivos:
            if arq.endswith(".txt"):
                # Identifica o par pelo prefixo (ex: "11" e "12" → par "1")
                if arq[1] == "1":  # Canal 1
                    chave = arq[0]
                    pares.setdefault(chave, {})["c1"] = arq
                elif arq[1] == "2":  # Canal 2
                    chave = arq[0]
                    pares.setdefault(chave, {})["c2"] = arq
        ...
        fs1, dados1, meta1 = ler_txt_vallen(os.path.join(pasta_material, par.get("c1", "")))
        fs2, dados2, meta2 = ler_txt_vallen(os.path.join(pasta_material, par.get("c2", "")))

        st.subheader(f"📂 Experimento: {material} — Par {par_sel}")
        ...
        if "time_raw" in meta1 and "time_raw" in meta2:
            ...
            if t1 is not None and t2 is not None:
                ...
                toa_info = (
                    ...
                    f"**Δt:** `{delta_t*1e6:.2f} µs`  \n"
                    f"**Distância estimada da fonte:** `{distancia*100:.2f} cm` "
                    f"(usando v = {vel} m/s)"
                )

        if toa_info:
            with st.expander("⏱️ Time of Arrival (ToA) — Localização da Fonte"):
                st.markdown(toa_info)
        ...
        for col, dados, fs, meta, label in [
            ...
            with col:
                ...
                axes[0].axhline(threshold_mv, color="red", linestyle="--", linewidth=0.8, label=f"Threshold ({threshold_mv} mV)")
    ...
    # ============================================================
    # MODO: COMPARAÇÃO ENTRE MATERIAIS
    ...
    # ============================================================
        ...
        for pasta in subpastas:
            ...
            if todas_features:
                ...
                # Calcula média e desvio padrão de cada feature para o material
        ...
        if resumo:
            ...
            for col_feat in df_comp.columns:
                ...
                ax.bar(df_comp.index, df_comp[col_feat], color="steelblue", edgecolor="black")
    ...
    # ============================================================
        ...
        if registros:
            ...
            # Reordena colunas para Material e Arquivo aparecerem primeiro
